...
import TeamLeadComplaintCards from './components/TeamLeadComplaintCards';
import TeamManagerComplaintCards from './components/TeamManagerComplaintCards';
import { forwardComplaint,fetchComplaints } from './complaints/actions/EmpComplaintActions';
...
const Sidebar = ({ isDarkMode, onMenuClick ,setActiveSection}) => {
  ...
  const menuItems = [
    ...
    { icon: <Users size={20} />, label: 'Complaints to solve',
     onClick:()=>setActiveSection('TeamManagerComplaints')
  },
    {
      icon: <Plus size={20} />,
...
const TeamManagerDashboard = () => {
  ...
  useEffect(() => {
    if (activeSection === "TeamManagerComplaints") {
  ...
  return (
    <div className={`flex ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <div className="flex-1">
        ...
        <div className="p-6">
        {activeSection==='TeamManagerComplaints' && (
              <TeamManagerComplaintCards />
            ) }
